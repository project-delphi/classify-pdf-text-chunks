...
class VectorStoreConfig:
    ...
    embedding_model: str = (
        "sentence-transformers/all-MiniLM-L6-v2"  # Embedding model to use
    )
...
def refill_tokens():
    ...
    with RATE_LIMIT_LOCK:
        ...
        RATE_LIMIT_CURRENT_TOKENS = min(
            float(RATE_LIMIT_TOKENS), RATE_LIMIT_CURRENT_TOKENS + new_tokens
        )
...
def initialize_vector_store() -> None:
    ...
    try:
        ...
        vector_store = FAISS.from_texts(
            [""], embeddings, metadatas=[{"doc_id": "empty"}]
        )
